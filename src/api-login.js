 ...
 * KHÔNG cần OCR/captcha. Nhanh hơn Playwright nhiều (1 HTTP round-trip).
 *
 * Vì sao bỏ được captcha: trang login dùng `react-simple-captcha` validate hoàn toàn
 * phía client. Endpoint POST /api/auth/login chỉ yêu cầu field `Otp` non-empty (không
 * verify được giá trị do client tự sinh), nên gửi Otp bất kỳ là qua.
 *
 * Model login: { UserName, Password, AgentCode, Otp }. Body mã hoá AES-128-CBC như mọi
 * request Muadi. Response JSON thường: { accessToken, refreshToken, memberInfo, agentInfo,
...
// Otp = ô captcha client-side; server chỉ cần non-empty. Cho random để không gửi giá trị cố định.
function randomOtp() {
